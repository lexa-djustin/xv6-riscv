  ...
  while (n > 0) {
    ...
    while (cons.r == cons.w) {
      if (signal_pending(myproc()) ||
          sleep_interruptible(&cons.r, &cons.lock)) {
        release(&cons.lock);
        return -ERESTARTSYS;
      }
    }
...
consoleintr(int c)
{
  ...
  switch (c) {
  case C('C'): { // Terminal-generated SIGINT.
    int pgid = cons_foreground_pgid;
    consputc('^');
    consputc('C');
    consputc('\n');
    while(cons.e != cons.w)
      cons.e--;
    int delivered = pgid > 0
      ? signal_send_pgrp(pgid, SIGINT, 0, SIGNAL_CODE_TTY) : 0;
    if(delivered == 0 && cons.e - cons.r < INPUT_BUF_SIZE) {
